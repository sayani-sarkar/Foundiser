...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * from user1 where id=$from";
    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    $sql = "SELECT * from user1 where id=$to";
    $query = mysqli_query($conn,$sql);
    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $sql = "UPDATE user1 set balance=$newbalance where id=$from";
                mysqli_query($conn,$sql);


                // adding amount to reciever's account
                $newbalance = $sql2['balance'] + $amount;
                $sql = "UPDATE user1 set balance=$newbalance where id=$to";
                mysqli_query($conn,$sql);
	...
	<div class="container">
        <h2 class="text-center pt-4" style="color : black;">Transaction</h2>
            <?php
                include 'config.php';
                $sid=$_GET['id'];
                $sql = "SELECT * FROM  user1 where id=$sid";
